...
export function makeBoardRetry(maxEmptyCount: number, tries: number) {
    let bestBoard = makeBoard(maxEmptyCount)
    let bestCnt = emptyCount(bestBoard);
    for (let i = 1; i < tries; i++) {
        let board = makeBoard(maxEmptyCount);
        let cnt = emptyCount(board);
        if (cnt > bestCnt) {
            bestBoard = board;
            bestCnt = cnt;
        }
